...
def _parse_global_header(fp, fs):
    ...
    _ = _read_cstring(fp, d)         # header checksum (already positioned after it)
    return {
...
def _index_json_and_checks(vals):
    # layout flex: either 5-json-fields or 4-json-fields variant
    if len(vals) < 25:
...
def pack_iter_neo(items, outfile=None, formatspecs=None,
    ...
    dst = _Dst()
    _write_global_header(dst, fs, len(list(items)), encoding, checksumtypes[0])
