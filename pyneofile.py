...
if PY2:
    ...
    def u(s): return s.decode('utf-8') if isinstance(s, bytes) else s
else:
    import zlib as _zlib
    try:
    ...
    import bz2 as _bz2
    import hashlib as _hashlib
    def b(s): return s if isinstance(s, (bytes, bytearray)) else str(s).encode('utf-8')
    def u(s): return s.decode('utf-8') if isinstance(s, (bytes, bytearray)) else str(s)

# Octal-safe mode constants
def _O(s):
    try: return int(s, 8)
    except Exception: return int(str(s), 8)
S_IFREG = _O('0100000')
S_IFDIR = _O('0040000')
...
def _hex(n): return format(int(n), 'x')
...
def _checksum(data, cstype, text=False):
    ...
    if t in ('', 'none'): return '0'
    if not isinstance(data, (bytes, bytearray)):
        data = b(data)
    if t == 'crc32': return _crc32_hex(data)
    if t in ('md5','sha1','sha224','sha256','sha384','sha512'):
        h = getattr(_hashlib, t)(); h.update(data); return h.hexdigest()
...
def _compress_bytes(data, algo='none', level=None):
    ...
    if not isinstance(data, (bytes, bytearray)): data = b(data)
    if algo == 'none': return data, 'none'
    if algo == 'zlib':
    ...
    if algo == 'bz2':
        ...
        return _bz2.compress(data, lvl), 'bz2'
    if algo == 'lzma':
        if _lzma is None: raise ValueError('lzma not available')
        if level is None: return _lzma.compress(data), 'lzma'
...
def _default_formatspecs():
    return {'format_magic': 'NeoFile', 'format_ver':'001', 'format_delimiter': b('\\x00'), 'new_style': True}
...
def _load_formatspecs_from_ini(paths=None, prefer_section=None):
    ...
    with io.open(picked, 'r', encoding='utf-8', errors='ignore') as f:
        (cp.read_file(f) if hasattr(cp, 'read_file') else cp.readfp(f))
    if prefer_section and cp.has_section(prefer_section): sec = prefer_section
    ...
    return {'format_magic':magic,'format_ver':_ver_digits(ver),'format_delimiter': b(_decode_escape(delim)),
            'new_style': str(newst).lower() in ('true','1','yes'), 'format_name': sec}

def _ensure_formatspecs(specs=None):
    if specs and isinstance(specs, dict):
        fs = dict(_default_formatspecs()); fs.update(specs); return fs
    env = _load_formatspecs_from_ini(); return env or _default_formatspecs()
...
def _write(bufmode, fp, buf, data):
    if not isinstance(data, (bytes, bytearray)): data = b(data)
    if bufmode: buf.extend(data)
    else: fp.write(data)

def _read_cstring(fp, delim):
    """Robust, streaming cstring read up to delimiter (handles multi-byte)."""
    d = delim; dl = len(d); read = fp.read
    if dl == 1:
        out = bytearray()
        while True:
            chunk = read(4096)
            ...
            if idx != -1:
                ...
                tail = chunk[idx+1:]
                if tail: fp.seek(-len(tail), os.SEEK_CUR)
                return bytes(out)
            out.extend(chunk)
    ...
    while True:
        ...
        if pos != -1:
            out.extend(data[:pos])
            rem = data[pos+dl:]
            if rem: fp.seek(-len(rem), os.SEEK_CUR)
            return bytes(out)
        # keep last dl-1 bytes to bridge boundary
        if dl > 1:
            keep_len = dl - 1
            if len(data) > keep_len:
                out.extend(data[:-keep_len])
                carry = bytearray(data[-keep_len:])
            else:
                carry = bytearray(data)
...
def _parse_global_header(fp, fs):
    ...
    _ = _read_cstring(fp, d)  # magic+ver
    ...
    for _i in range(extrafields): _ = _read_cstring(fp, d)
    checksumtype = u(_read_cstring(fp, d))
    _ = _read_cstring(fp, d)  # header checksum
    return {'fencoding': fencoding or 'UTF-8', 'fnumfiles': fnumfiles, 'fostype': fostype,
            'fextradata': [], 'fchecksumtype': checksumtype, 'ffilelist': [], 'fformatspecs': fs}

def _index_json_and_checks(vals):
    if len(vals) < 25: raise ValueError("Record too short: got %d" % len(vals))
    idx = 25
    def ishex(s):
        s = u(s)
        return bool(s) and all(c in '0123456789abcdefABCDEF' for c in s)
    v2 = vals[idx+1] if idx+1 < len(vals) else b''
    v3 = vals[idx+2] if idx+2 < len(vals) else b''
    v4 = vals[idx+3] if idx+3 < len(vals) else b''
    ...
    if ishex(v2) and ishex(v3) and u(v4).lower() in csnames:
        ...
        idx_json_type = 25; idx_json_len = None; idx_json_size = 26; idx_json_cst = 27; idx_json_cs = 28; idx_extras_size = 29
    idx_extras_count = idx_extras_size + 1
    count = int((vals[idx_extras_count] or b'0'), 16) if idx_extras_count < len(vals) else 0
    idx = idx_extras_count + 1 + count
    return (idx_json_type, idx_json_len, idx_json_size, idx_json_cst, idx_json_cs,
            idx, idx+1, idx+2, idx+3)  # header_cs_type, content_cs_type, header_cs, content_cs
...
def _parse_record(fp, fs, listonly=False, skipchecksum=False, uncompress=True, skipjson=False):
    ...
    if first == b'0':
        ...
        if second == b'0': return None  # end marker
        headersize_hex = first; fields_len_hex = second
    else:
        headersize_hex = first; fields_len_hex = _read_cstring(fp, d)
    ...
    if fjsonsize > 0:
        if listonly or skipjson:
            fp.seek(fjsonsize + len(d), os.SEEK_CUR)
            json_bytes = b''
        else:
            json_bytes = fp.read(fjsonsize); fp.read(len(d))
    else:
        json_bytes = b''; fp.read(len(d))
    ...
    content_stored = b''
    if stored_len:
        if listonly: fp.seek(stored_len, os.SEEK_CUR)
        else: content_stored = fp.read(stored_len)
    fp.read(len(d))

    header_cs_type  = u(vals[idx_header_cs_type]) if idx_header_cs_type < len(vals) else 'none'
    content_cs_type = u(vals[idx_content_cs_type]) if idx_content_cs_type < len(vals) else 'none'
    header_cs_val   = u(vals[idx_header_cs]) if idx_header_cs < len(vals) else '0'
    content_cs_val  = u(vals[idx_content_cs]) if idx_content_cs < len(vals) else '0'
    json_cs_type    = u(vals[idx_json_cst]) if idx_json_cst < len(vals) else 'none'
    json_cs_val     = u(vals[idx_json_cs]) if idx_json_cs < len(vals) else '0'
    ...
    if (not listonly) and uncompress and u(fcompression) not in ('', 'none') and content_stored:
        try: content_ret = _decompress_bytes(content_stored, u(fcompression))
        except Exception: content_ret = content_stored
    ...
    fjson = {}
    if json_bytes:
        try: fjson = json.loads(u(json_bytes))
        except Exception: fjson = {}

    to_int = lambda x: int((x or b'0'), 16)
    return {'fid': to_int(fid_hex), 'finode': to_int(finode_hex), 'fname': name, 'flinkname': u(flinkname),
            'ftype': to_int(ftypehex), 'fsize': to_int(fsize_hex), 'fcsize': to_int(fcsize_hex),
            'fatime': to_int(fatime_hex), 'fmtime': to_int(fmtime_hex), 'fctime': to_int(fctime_hex), 'fbtime': to_int(fbtime_hex),
            'fmode': to_int(fmode_hex), 'fwinattributes': to_int(fwinattrs_hex), 'fuid': to_int(fuid_hex), 'funame': u(funame),
            'fgid': to_int(fgid_hex), 'fgname': u(fgname), 'fcompression': u(fcompression) or 'none',
            'fseeknext': u(fseeknextfile), 'fjson': fjson, 'fcontent': (None if listonly else content_ret)}
...
def _append_null(x, d): return (x if isinstance(x, (bytes, bytearray)) else b(x)) + d
def _append_nulls(arr, d):
    out = bytearray()
    for it in arr: out.extend(_append_null(it, d))
    return bytes(out)

def _write_global_header(dst, fs, numfiles, encoding, checksumtype):
    d = fs['format_delimiter']; magic = fs['format_magic']; ver = _ver_digits(fs['format_ver'])
    platform_name = os.name
    extras = []; extrafields = _hex(len(extras))
    extras_blob = _append_null(extrafields, d) + (b'' if not extras else _append_nulls(extras, d))
    body = _append_nulls([ _hex(3 + 5 + len(extras) + 1), encoding, platform_name,
                           _hex(int(numfiles)), _hex(len(extras_blob)), extrafields ], d)
    if extras: body += _append_nulls(extras, d)
...
def _build_record(fs, meta, jsondata, content_stored, checksumtypes):
    d = fs['format_delimiter']; H = lambda x: _hex(int(x))
    fname = meta['fname'];  fname = ('./' + fname) if not fname.startswith(('./','/')) else fname
    now = int(time.time())
    fields = [
        H(meta.get('ftype', 0)),
        ...
        meta.get('flinkname',''),
        H(meta.get('fsize',0)),
        H(meta.get('fatime', now)), H(meta.get('fmtime', now)), H(meta.get('fctime', now)), H(meta.get('fbtime', now)),
        H(meta.get('fmode', S_IFREG | MODE_FILE_DEFAULT)),
        H(meta.get('fwinattributes',0)),
        meta.get('fcompression','none'),
        H(meta.get('fcsize', 0)),
        H(meta.get('fuid', 0)), meta.get('funame',''),
        H(meta.get('fgid', 0)), meta.get('fgname',''),
        H(meta.get('fid', meta.get('index', 0))), H(meta.get('finode', meta.get('index', 0))),
        H(meta.get('flinkcount', 1)), H(meta.get('fdev', 0)), H(meta.get('fdev_minor', 0)), H(meta.get('fdev_major', 0)),
        H(len(d)),
    ]
    if jsondata:
        raw_json = b(json.dumps(jsondata, separators=(',',':')))
        fjsontype, fjsonlen_hex, fjsonsize_hex = 'json', H(len(jsondata)), H(len(raw_json))
        fjson_cs_type = checksumtypes[2]; fjson_cs = _checksum(raw_json, fjson_cs_type, True)
    else:
        raw_json = b''; fjsontype, fjsonlen_hex, fjsonsize_hex = 'none','0','0'
        fjson_cs_type, fjson_cs = 'none', '0'

    extras_size_hex = _hex(len(_append_null('0', d))); extrafields = '0'
    ...
    record_fields_len_hex = _hex(len(rec_fields) + 2)  # include checksum VALUE fields that follow
    ...
    headersize_hex = _hex(len(tmp_with_placeholders) - len(d))
...
def pack_iter_neo(items, outfile=None, formatspecs=None, checksumtypes=('crc32','crc32','crc32'),
    ...
    dst = _Dst()
    _write_global_header(dst, fs, len(items), encoding, checksumtypes[0])
    fid = 0
    for it in items:
        if isinstance(it, dict) and 'name' in it:
            name = it['name']; is_dir = bool(it.get('is_dir'))
            data = it.get('data'); mode = int(it.get('mode', S_IFDIR | MODE_DIR_DEFAULT if is_dir else S_IFREG | MODE_FILE_DEFAULT))
        ...
        name = u(name).replace('\\\\','/');  name = ('./' + name) if not name.startswith(('./','/')) else name
        if is_dir or name.endswith('/'): raw = b''; ftype = 5
        else: raw = b'' if data is None else (data if isinstance(data, (bytes, bytearray)) else b(data)); ftype = 0
        algo = (compression or 'none').lower()
        if algo == 'auto': algo, lvl = _auto_pick_for_size(len(raw)); level = compression_level if compression_level is not None else lvl
        else: level = compression_level
        try: stored, used_algo = _compress_bytes(raw, algo, level)
        except Exception: stored, used_algo = _compress_bytes(raw, 'zlib', 6 if level is None else level)
        meta = {'ftype': ftype, 'fencoding': encoding, 'fcencoding': encoding, 'fname': name, 'flinkname':'',
                'fsize': len(raw), 'fatime': mtime, 'fmtime': mtime, 'fctime': mtime, 'fbtime': mtime,
                'fmode': int(mode), 'fwinattributes': 0, 'fcompression': used_algo, 'fcsize': len(stored),
        ...
        _write(bufmode, fp, buf, rec); fid += 1
    _write(bufmode, fp, buf, _append_nulls(['0','0'], fs['format_delimiter']))
    if bufmode: return bytes(buf)
    return True

def pack_neo(infiles, outfile=None, formatspecs=None, checksumtypes=('crc32','crc32','crc32'),
             encoding='UTF-8', compression='auto', compression_level=None):
    ...
    for p in paths:
        ...
        if os.path.isdir(p):
            for root, dirs, files in os.walk(p):
                ...
                for dname in dirs:
                    rel = os.path.join(relroot, dname).replace('\\\\','/');  rel = (rel if rel.endswith('/') else (rel + '/')) if rel else rel
    ...
    for apath, is_dir, relname in filelist:
        if is_dir: raw = b''; ftype = 5
        else:
            with io.open(apath, 'rb') as f: raw = f.read()
            ftype = 0
        relname = ('./' + relname) if not relname.startswith(('./','/')) else relname
        algo = (compression or 'none').lower()
        if algo == 'auto': algo, lvl = _auto_pick_for_size(len(raw)); level = compression_level if compression_level is not None else lvl
        else: level = compression_level
        try: stored, used_algo = _compress_bytes(raw, algo, level)
        ...
        now = int(time.time())
        meta = {'ftype': ftype, 'fencoding': encoding, 'fcencoding': encoding, 'fname': relname, 'flinkname':'',
                'fsize': len(raw), 'fatime': now, 'fmtime': now, 'fctime': now, 'fbtime': now,
                'fmode': (S_IFDIR | MODE_DIR_DEFAULT) if is_dir else (S_IFREG | MODE_FILE_DEFAULT), 'fwinattributes': 0,
                'fcompression': used_algo, 'fcsize': len(stored), 'fuid':0,'funame':'','fgid':0,'fgname':'',
...
def archivefilelistfiles_neo(infile, formatspecs=None, advanced=False, include_dirs=True, skipjson=True):
    ...
    try:
        ...
        while True:
            ...
            is_dir = (rec['ftype'] == 5)
            if not advanced:
                if is_dir and not include_dirs: continue
                out.append(rec['fname'])
            else:
                out.append({'name': rec['fname'], 'type': 'dir' if is_dir else 'file',
                            'compression': rec['fcompression'] or 'none', 'size': rec['fsize'],
                            'stored_size': rec['fcsize'], 'mtime': rec['fmtime'], 'atime': rec['fatime'],
                            'mode': rec['fmode']})
        return out
    finally:
        if need_close:
...
def archivefilevalidate_neo(infile, formatspecs=None, verbose=False, return_details=False, skipjson=False):
    ...
    if return_details: return ok, details
    return ok
...
def unpack_neo(infile, outdir='.', formatspecs=None, skipchecksum=False, uncompress=True):
    ...
    if outdir in (None, '-', b'-'):
        ...
        for ent in arr.get('ffilelist', []):
            result[ent['fname']] = None if ent['ftype'] == 5 else (ent.get('fcontent') or b'')
        return result
    if not os.path.isdir(outdir):
        if os.path.exists(outdir): raise ValueError('not a directory: %r' % outdir)
        os.makedirs(outdir)
    for ent in arr.get('ffilelist', []):
        ...
        if ent['ftype'] == 5:
            ...
            with io.open(path, 'wb') as f: f.write(ent.get('fcontent') or b'')
            try: os.chmod(path, ent.get('fmode', 0) & 0o777)
            except Exception: pass
    return True
...
def repack_neo(infile, outfile=None, formatspecs=None, checksumtypes=('crc32','crc32','crc32'),
    ...
    for ent in arr.get('ffilelist', []):
        ...
        try: raw = _decompress_bytes(stored_src, src_algo) if src_algo != 'none' else stored_src
        except Exception: raw = stored_src
        dst_algo = (compression or 'auto').lower()
        if dst_algo == 'auto': dst_algo, dst_level = _auto_pick_for_size(len(raw))
        ...
        if dst_algo == src_algo or (dst_algo=='none' and src_algo=='none'):
            ...
            try: raw_len = len(_decompress_bytes(stored_src, src_algo)) if src_algo != 'none' else len(stored_src)
            except Exception: raw_len = len(stored_src)
        else:
            stored, used_algo = _compress_bytes(raw, dst_algo, dst_level); raw_len = len(raw)
        now = int(time.time())
        meta = {'ftype': ent['ftype'], 'fencoding': arr.get('fencoding','UTF-8'), 'fcencoding': arr.get('fencoding','UTF-8'),
                'fname': ent['fname'], 'flinkname': ent.get('flinkname',''), 'fsize': raw_len,
                'fatime': ent.get('fatime', now), 'fmtime': ent.get('fmtime', now), 'fctime': ent.get('fctime', now), 'fbtime': ent.get('fbtime', now),
                'fmode': ent.get('fmode', S_IFREG | MODE_FILE_DEFAULT), 'fwinattributes': ent.get('fwinattributes', 0),
                'fcompression': used_algo, 'fcsize': len(stored), 'fuid': ent.get('fuid', 0), 'funame': ent.get('funame',''),
                'fgid': ent.get('fgid', 0), 'fgname': ent.get('fgname',''), 'fid': ent.get('fid', out_count), 'finode': ent.get('finode', out_count),
                'flinkcount': ent.get('flinkcount', 1), 'fdev': ent.get('fdev', 0), 'fdev_minor': ent.get('fdev_minor', 0), 'fdev_major': ent.get('fdev_major', 0),
                'index': out_count}
        rec = _build_record(fs, meta, ent.get('fjson', {}), stored, checksumtypes)
        _write(bufmode, fp, buf, rec); out_count += 1
    _write(bufmode, fp, buf, _append_nulls(['0','0'], fs['format_delimiter']))
    if bufmode: return bytes(buf)
    return True

def convert_foreign_to_neo(infile, outfile=None, formatspecs=None, checksumtypes=('crc32','crc32','crc32'),
                           compression='auto', compression_level=None):
    if isinstance(infile, (bytes, bytearray)): raise ValueError("convert expects a file path")
    p = u(infile); lp = p.lower()
    if lp.endswith('.zip'):
        import zipfile
        z = zipfile.ZipFile(p, 'r'); items = []
        for zi in z.infolist():
            name = zi.filename; is_dir = name.endswith('/')
